...
class KnowledgeBoard {
  ...
  async createDisputeCard(item) {
    ...
    // Idempotent by title key: the steward (and any future producer) may
    // re-detect the same pair on every rotation — one pair, one card.
    const cardTitle = `Dispute: ${item.title}`;
    try {
      const existingCards = await this.deck.getCardsInStack('disputed');
      const duplicate = existingCards.find(c => c.title === cardTitle);
      if (duplicate) {
        console.log(`[KnowledgeBoard] Skipping duplicate dispute: ${item.title}`);
        return duplicate;
      }
    } catch (error) {
      // If we cannot check for duplicates, proceed with creation
      console.warn('[KnowledgeBoard] Could not check for dispute duplicates:', error.message);
    }
...
*Auto-generated by Moltagent*`;
  ...
  /**
   * Create a card for stale/compost-candidate knowledge.
   * Placed in the 'stale' stack. Idempotent by title key.
   *
   * @param {Object} item
   * @param {string} item.title - The page or fact that went stale
   * @param {string} item.description - Why it is considered stale, plus links
   * @returns {Promise<Object>} Created card object, or existing duplicate
   */
  async createStaleCard(item) {
    await this.initialize();

    const cardTitle = `Stale: ${item.title}`;
    try {
      const existingCards = await this.deck.getCardsInStack('stale');
      const duplicate = existingCards.find(c => c.title === cardTitle);
      if (duplicate) {
        console.log(`[KnowledgeBoard] Skipping duplicate stale card: ${item.title}`);
        return duplicate;
      }
    } catch (error) {
      console.warn('[KnowledgeBoard] Could not check for stale duplicates:', error.message);
    }

    const duedate = this._addDays(new Date(), this.config.verificationDueDays);

    const card = await this.deck.createCard('stale', {
      title: cardTitle,
      description: `## Stale Knowledge

${item.description}

### Please Review

Still relevant? Move to "Verified". Ready to archive? Leave here — composting picks it up.
...
*Auto-generated by Moltagent*`,
    ...
    console.log(`[KnowledgeBoard] Created stale card: ${card.id}`);
    return card;
  }
